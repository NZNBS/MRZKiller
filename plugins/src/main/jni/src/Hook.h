...
static monoString *FormatCount(int enemy, int bot) {
    char buf[128] = {};
    sprintf(buf, "[Enemy:%d Bot:%d]", enemy, bot);
    return U3DStr(buf);
}
...
static bool IsFalling(void *player) {
    bool (*_IsFalling)(void *players) = (bool (*)(void *))getRealOffset(Global.IsFalling);
    return _IsFalling(player);
}
...
static void spofNick(void *players) {
    ...
    _spof_nick(players, U3DStr("MRZ - TEAM"));
}
...
void *(*Player_HeadCollider)(void * _this) = (void *(*)(void *))getRealOffset(0xB6FF70);
void* (*Component_get_gameObject)(void *_this) = (void*(*)(void *))getRealOffset(0x2469B7C);
int (*GameObject_get_layer)(void * _this) = (int (*)(void *))getRealOffset(0x2426910);
bool (*Physics_Raycast)(void * ptr, Vector3 origin, Vector3 direction, RayCastHit * hitInfo, float maxDistance) = (bool (*)(void *, Vector3, Vector3, RayCastHit *, float))getRealOffset(0x275FD7C);
...
bool isVisible(Vector3 target, Vector3 camera, float distance)
{
...
static Quaternion get_aim(void *player) {
    Quaternion (*_get_aim)(void *players) = (Quaternion (*)(void *))getRealOffset(0xB58B54);
    return _get_aim(player);
}
